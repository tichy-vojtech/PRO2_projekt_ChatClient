...
public class ApiChatClient implements ChatClient { //http://fimuhkpro22021.aspifyhost.cz/swagger/index.html
    ...
    public ApiChatClient() {
        ...
        Runnable refreshData = () ->{ //vytvoření nového vlákna
        ...
        Thread refreshDataThread = new Thread(refreshData); //jeho spuštění
    ...
    private void refreshLoggedUsers() {
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                loggedUsers = gson.fromJson(resultJson, new TypeToken<ArrayList<String>>() {
                }.getType());
                raiseEventLoggedUsersChanged();
            }
        } catch (Exception e) {
    ...
    private void refreshMessages() {
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                messages = gson.fromJson(resultJson, new TypeToken<ArrayList<Message>>() {
                }.getType());
                raiseEventMessagesChanged();
            }
        } catch (Exception e) {
